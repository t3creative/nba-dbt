...
unpivoted_data as (
    ...
    {% for slug in iterate_slugs %}
        ...
        {% if execute %}
        {{ log("PPM_LOOP_DEBUG: Slug: " ~ slug ~ ", OverOddsCol: " ~ over_odds_col ~ ", InUpstream: " ~ (over_odds_col in upstream_col_names) ~ ", IsConsensus: " ~ (slug == consensus_slug_var) ~ ", IsPlaceholder: " ~ (slug == placeholder_slug), info=True) }}
        {% endif %}

        {% if slug != placeholder_slug and slug != consensus_slug_var and over_odds_col in upstream_col_names %}
    ...
    from base_data
    where base_data.{{ over_odds_col }} is not null or base_data.{{ under_odds_col }} is not null
    ...
    select {# Ensure this select is on a very new line, isolated #}

        null::text as player_prop_key, null::date as game_date, null::text as player_name, null::text as player_id, null::text as player_slug,
        null::text as market, null::text as market_id, null::numeric as line,
        null::numeric as consensus_over_odds_decimal, null::numeric as consensus_under_odds_decimal,
        null::numeric as consensus_over_implied_prob, null::numeric as consensus_under_implied_prob,
        null::numeric as consensus_total_implied_prob, null::numeric as consensus_over_no_vig_prob,
        null::numeric as consensus_under_no_vig_prob, null::numeric as consensus_hold_percentage,
        null::text as sportsbook_slug, null::numeric as book_over_odds, null::numeric as book_under_odds,
        null::numeric as book_over_implied_prob, null::numeric as book_under_implied_prob,
        null::numeric as book_total_implied_prob, null::numeric as book_no_vig_over_prob,
        null::numeric as book_no_vig_under_prob, null::numeric as book_hold_percentage
    where 1=0
...
source_props_enhanced as (
    select
        ...
        case 
            when book_no_vig_over_prob > 0 and book_no_vig_over_prob < 1 then
                case
                    when book_no_vig_over_prob >= 0.5 then - (book_no_vig_over_prob / (1 - book_no_vig_over_prob)) * 100
                    else ( (1 - book_no_vig_over_prob) / book_no_vig_over_prob) * 100
                end
            else null
        end as fair_over_odds_american,
        case 
            when book_no_vig_under_prob > 0 and book_no_vig_under_prob < 1 then
                case
                    when book_no_vig_under_prob >= 0.5 then - (book_no_vig_under_prob / (1 - book_no_vig_under_prob)) * 100
                    else ( (1 - book_no_vig_under_prob) / book_no_vig_under_prob) * 100
                end
            else null
        end as fair_under_odds_american,
        {{ dbt_utils.generate_surrogate_key(['player_prop_key', 'sportsbook_slug']) }} as market_analysis_key
    from unpivoted_data
),
...
market_context as (
    select
        ...
        avg(book_total_implied_prob) as avg_market_total_implied_prob, 
        avg(book_hold_percentage) as avg_market_hold_percentage    
...
sportsbook_context as (
    select
        ...
        avg(book_total_implied_prob) as avg_sportsbook_total_implied_prob,
        avg(book_hold_percentage) as avg_sportsbook_hold_percentage,
        stddev(book_hold_percentage) as stddev_sportsbook_hold_percentage
    from source_props_enhanced
...
final as (
    select
        ...
        spe.line - mc.avg_market_line as line_vs_market_avg,
        spe.line - pc.avg_player_market_line as line_vs_player_avg,
        spe.book_hold_percentage - sc.avg_sportsbook_hold_percentage as hold_vs_book_avg,
        spe.book_hold_percentage - mc.avg_market_hold_percentage as hold_vs_market_avg,
        spe.game_date,
        current_timestamp as analysis_timestamp
    from source_props_enhanced spe
    left join market_context mc on spe.market_id = mc.market_id 
    left join player_context pc on spe.player_id = pc.player_id and spe.market_id = pc.market_id
