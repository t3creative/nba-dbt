...
- Purpose: Provides context for a specific game matchup based on season-long team performance ranks and historical head-to-head results.

DATA TEMPORALITY & FEATURE TYPE
--------------------------------------------------
- Contains Current Event Data: YES
    * Season ranks and averages are calculated over the entire season up to the point the model is run, potentially including the current game's data.

- Historical Data Context (Details if data from prior events is used):
    - Type: Cumulative (Season-to-Date for ranks/averages), Full History Preceding Event (for head-to-head matchups)
    - Window Size (if Rolling): N/A
    - Window Basis (Defines what constitutes the window): Team's games this season (for ranks/averages), All previous head-to-head games (for matchup history)
    - Note: N/A

- Primary Use for ML Prediction: NO
    * Due to the inclusion of season ranks/averages that may contain current game data, this model is not suitable for predicting the outcome of the current game without data leakage.
