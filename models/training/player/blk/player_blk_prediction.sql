...
with player_props as (
    ...
    and pp.market = 'Blocks O/U' -- Filter for BLK market only
),
...
combined_features as (
    select
        ...
        opd.avg_blk_allowed_to_position,

        -- Numerical matchup quality indicators (_vs_league_avg)
        opd.blk_vs_league_avg,
...
select
    ...
    -- UPDATED: Opponent position defense features (BLK only)
    round(avg_blk_allowed_to_position::numeric, 3) as opp_avg_blk_allowed_to_position,

    -- Numerical matchup quality indicators (_vs_league_avg) (BLK only)
    round(blk_vs_league_avg::numeric, 3) as opp_blk_vs_league_avg,

    -- Locally generated Categorical matchup labels (BLK only)
    CASE
        WHEN COALESCE(blk_vs_league_avg, 0.0) > 0.5 THEN 'Great'
        WHEN COALESCE(blk_vs_league_avg, 0.0) > 0.2 THEN 'Good'
        WHEN COALESCE(blk_vs_league_avg, 0.0) >= -0.2 THEN 'Average'
        WHEN COALESCE(blk_vs_league_avg, 0.0) >= -0.5 THEN 'Poor'
        ELSE 'Bad'
    END AS opp_blk_matchup_label,
