...
usage_patterns AS (
    ...
    FROM {{ ref('feat_player__usage_rolling') }}
),
...
derived_features AS (
    SELECT
        ...
        CASE 
            ...
            THEN EXTRACT(DAY FROM (bb.game_date::date - ms.latest_30_plus_pt_game_date::date))
            ELSE NULL 
        END AS days_since_30_plus_pt_game,
        
        CASE 
            WHEN ms.latest_40_plus_pt_game_date IS NOT NULL 
            THEN EXTRACT(DAY FROM (bb.game_date::date - ms.latest_40_plus_pt_game_date::date))
            ELSE NULL 
        ...
        -- Standard deviation of points in recent games
        STDDEV(bb.pts) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS pts_roll_5g_stddev,
        
        -- Efficiency rolling features
        AVG(bb.ts_pct) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS ts_pct_roll_5g_avg,
        
        AVG(bb.eff_fg_pct) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS eff_fg_pct_roll_5g_avg,
        
        -- Usage pattern rolling features
        AVG(bb.usage_pct) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING
        ) AS usage_pct_roll_3g_avg,
        
        AVG(bb.usage_pct) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS usage_pct_roll_5g_avg,
        
        -- Scoring efficiency features
        AVG(bb.pts / NULLIF(bb.min, 0)) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS pts_per_min_roll_5g_avg,
        
        AVG(bb.pts / NULLIF(bb.fga, 0)) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS pts_per_fga_roll_5g_avg,
        
        -- Shooting tendency features
        AVG(bb.fg3a / NULLIF(bb.fga, 0)) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS three_pt_tendency_roll_5g_avg,
        
        -- Free throw tendency feature
        AVG(bb.fta / NULLIF(bb.fga, 0)) OVER (
            PARTITION BY bb.player_id 
            ORDER BY bb.game_date, bb.game_id
            ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
        ) AS ft_rate_roll_5g_avg,
        ...
        CASE
            WHEN ms.latest_30_plus_pt_game_date IS NOT NULL AND cs.career_games_prior_game > 0 THEN
                EXTRACT(DAY FROM (bb.game_date::date - ms.latest_30_plus_pt_game_date::date)) / 
    ...
    FROM base_boxscores bb
    LEFT JOIN milestone_stats ms
        ON bb.player_game_key = ms.player_game_key
    LEFT JOIN usage_patterns up
        ON bb.player_game_key = up.player_game_key
    LEFT JOIN career_stats cs
        ON bb.player_game_key = cs.player_game_key
    LEFT JOIN derived_scoring_metrics dsm
        ON bb.player_game_key = dsm.player_game_key
)
