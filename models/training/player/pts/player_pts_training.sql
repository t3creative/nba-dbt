WITH traditional_boxscores AS (
  SELECT
    *
  FROM {{ ref('int__player_bxsc_traditional') }} AS player_traditional_boxscores
), advanced_boxscores AS (
  SELECT
    *
  FROM {{ ref('int__player_bxsc_advanced') }} AS player_advanced_boxscores
), hustle_boxscores AS (
  SELECT
    *
  FROM {{ ref('int__player_bxsc_hustle') }} AS player_hustle_boxscores
), usage_boxscores AS (
  SELECT
    *
  FROM {{ ref('int__player_bxsc_usage') }} AS player_usage_boxscores
), scoring_boxscores AS (
  SELECT
    *
  FROM {{ ref('int__player_bxsc_scoring') }} AS player_scoring_boxscores
), team_game_context /* Get team context features from int_game__schedules */ AS (
  SELECT
    *
  FROM {{ ref('int_game__schedules') }} AS int_game__schedules
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'away'
      ELSE NULL
    END AS home_or_away, /* Team context features from int_game__schedules */ /* Is this player on the home team or away team? */
    CASE
      WHEN tgc.home_team_id = pg.team_id
