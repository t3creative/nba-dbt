WITH traditional_boxscores AS (
  ...
  SELECT
    ...
    AVG(sbs.pct_pts_paint) OVER (PARTITION BY tbs.player_id ORDER BY tbs.game_date NULLS FIRST rows BETWEEN 4 preceding AND 1 preceding) AS pct_pts_paint_rolling_avg_4,
    ...
    */
    ...
    AVG(sbs.pct_pts_paint) OVER (PARTITION BY tbs.player_id ORDER BY tbs.game_date NULLS FIRST rows BETWEEN 8 preceding AND 1 preceding) AS pct_pts_paint_rolling_avg_8,
    ...
    AVG(sbs.pct_pts_paint) OVER (PARTITION BY tbs.player_id, tbs.season_year ORDER BY tbs.game_date NULLS FIRST rows BETWEEN 9999 preceding AND 1 preceding) AS pct_pts_paint_season_avg,
