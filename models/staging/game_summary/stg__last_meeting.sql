...
renamed AS (
    SELECT
        ...
        "LAST_GAME_HOME_TEAM_ABBREVIATION" AS last_game_home_team_abbreviation,
        CAST(
            "LAST_GAME_HOME_TEAM_POINTS" AS INTEGER
        ) AS last_game_home_team_points,
        CAST(
            "LAST_GAME_VISITOR_TEAM_ID" AS INTEGER
        ) AS last_game_visitor_team_id,
        "LAST_GAME_VISITOR_TEAM_CITY" AS last_game_visitor_team_city,
        "LAST_GAME_VISITOR_TEAM_NAME" AS last_game_visitor_team_name,
        "LAST_GAME_VISITOR_TEAM_CITY1" AS last_game_visitor_team_abbreviation,
        CAST(
            "LAST_GAME_VISITOR_TEAM_POINTS" AS INTEGER
        ) AS last_game_visitor_team_points
    FROM
        source
)
