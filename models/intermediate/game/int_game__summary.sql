...
MODEL OVERVIEW
--------------------------------------------------
- Entity: game, team, player, official
- Grain: game_id
- Purpose: Consolidates comprehensive game-level details, including scores, stats, historical context (last meeting, season series), inactive players, and officials.
...
- Key Source Models: ref('stg__game_summary'), ref('stg__game_info'), ref('stg__last_meeting'), ref('stg__season_series'), ref('stg__line_score'), ref('stg__other_stats'), ref('stg__inactive_players'), ref('stg__game_officials')
...
{{ config(
    ...
    cluster_by=['game_date_est', 'home_team_id', 'away_team_id'],
    indexes=[
        {'columns': ['game_id']},
...
final AS (
    SELECT
        ...
        lm.last_game_away_team_id,
        lm.last_game_away_team_points,
        ...
        ls_home.team_tricode AS home_team_tricode,
        ...
        ls_away.team_tricode AS away_team_tricode,
    ...
    LEFT JOIN line_score ls_away ON gs.game_id = ls_away.game_id AND gs.away_team_id = ls_away.team_id
