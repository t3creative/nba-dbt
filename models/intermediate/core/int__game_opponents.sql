WITH source_schedules AS (
  ...
  SELECT
    ...
    home_team_id,
    away_team_id,
    ROW_NUMBER() OVER (PARTITION BY game_id ORDER BY game_date DESC NULLS LAST) AS rn /* Add a row number partitioned by game_id, ordered by date descending */ /* to pick the most recent record for a game if duplicates exist */
