...
MODEL OVERVIEW
--------------------------------------------------
- Entity: team, opponent, game
- Grain: team_game (one row per team per game)
- Purpose: Provides context for a specific game matchup by calculating season-long and recent home/away performance metrics and differentials for both the team and its opponent.

DATA TEMPORALITY & FEATURE TYPE
--------------------------------------------------
- Contains Current Event Data: YES
    * Season averages and recent performance metrics are calculated over games up to the point the model is run, potentially including the current game's data.

- Historical Data Context (Details if data from prior events is used):
    - Type: Cumulative (Season-to-Date for home/away averages), Rolling (Last 5 games for recent performance)
    - Window Size (if Rolling): 5 games
    - Window Basis (Defines what constitutes the window): Team's games in specific home/away context (for rolling), Team's games this season (for cumulative)
    - Note: N/A

- Primary Use for ML Prediction: NO
    * Due to the inclusion of season/recent averages that may contain current game data, this model is not suitable for predicting the outcome of the current game without data leakage.
...
game_home_away_factors AS (
    SELECT
        ...
        go.team_id,
        go.opponent_id,
        -- Use the team_game_key from the source model
        go.team_game_key, -- Changed from generating MD5 game_team_key
        go.season_year,
        go.home_away,
