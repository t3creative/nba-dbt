...
SELECT
    ...
    opp.opp_l10_def_rating_prior AS opponent_defensive_rating,
    opp.opp_l10_pace_prior AS opponent_pace,
    opp.opp_adjusted_def_rating_prior AS opponent_adjusted_def_rating,
    
    -- Position matchup features
    pm.avg_pts_allowed_to_position,
    pm.pts_vs_league_avg,
    pm.pts_matchup_label,
    pm.reb_matchup_label,
    pm.ast_matchup_label,
    ...
    CASE
        WHEN pvo.sample_confidence_prior >= 4 THEN
            (pf.last5_avg_pts * 0.4) + 
            (pvo.recency_weighted_pts_prior * 0.4) + 
            (pm.avg_pts_allowed_to_position * 0.2)
        WHEN pvo.sample_confidence_prior >= 2 THEN
            (pf.last5_avg_pts * 0.6) + 
            (pvo.avg_pts_vs_opponent_prior * 0.2) + 
            (pm.avg_pts_allowed_to_position * 0.2)
        ELSE
    ...
    CASE
        WHEN pvo.sample_confidence_prior >= 4 THEN
            ...
            (pvo.avg_reb_vs_opponent_prior * 0.4) + 
            (pm.avg_reb_allowed_to_position * 0.2)
        WHEN pvo.sample_confidence_prior >= 2 THEN
            (pf.last5_avg_reb * 0.6) + 
            (pvo.avg_reb_vs_opponent_prior * 0.2) + 
            (pm.avg_reb_allowed_to_position * 0.2)
        ELSE
    ...
    CASE
        WHEN pvo.sample_confidence_prior >= 4 THEN
            ...
            (pvo.avg_ast_vs_opponent_prior * 0.4) + 
            (pm.avg_ast_allowed_to_position * 0.2)
        WHEN pvo.sample_confidence_prior >= 2 THEN
            (pf.last5_avg_ast * 0.6) + 
            (pvo.avg_ast_vs_opponent_prior * 0.2) + 
            (pm.avg_ast_allowed_to_position * 0.2)
        ELSE
